...
router.post('/createPayment', userAuthMiddleware, async (req, res) => {
  ...
  try {
    const [id] = await (await databaseManager.getKnex())('payments').insert({
      order_id,
      payment_method,
      status: 'completed',
    });
    ...
    await (await databaseManager.getKnex())('orders').where('id', order_id).update({
      is_paid: true,
      // is_delivered: true,
    });

    await (await databaseManager.getKnex())('products')
...
router.get('/getPaymentsByCustomerId', userAuthMiddleware, async (req, res) => {
  try {
    ...
    const payments = await (await databaseManager.getKnex())('payments')
